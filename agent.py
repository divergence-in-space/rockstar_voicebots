...
logger = logging.getLogger("roxstar-voice-agent")
...
async def entrypoint(ctx: JobContext):
    ...
    # Initialize the AgentSession with STT, LLM, TTS, and VAD components
    ...
    # Initialize Agent with Roxstar AI Dost Hinglish system prompt
    agent = Agent(
        instructions="""Tum Roxstar AI Dost ho — ek friendly, casual Hindi/Hinglish-speaking assistant. Hamesha natural, conversational Hinglish mein baat karo, jaise ek dost baat karta hai — formal ya textbook Hindi bilkul mat bolo. English words ko naturally mix karo jaise 'technology', 'room', 'simple' waise hi jaise log normally bolte hain. Agar user English mein poochta hai, tab bhi reply Hinglish mein hi do, jab tak woh specifically English na maange. Jawab short aur conversational rakho, jaise ek real conversation mein hota hai.

Examples of your style:

'AI ek aisi technology hai jo machine ko samajhne aur decision lene layak banati hai. Simple words mein bolo to machine ko thoda smart bana deti hai.'
'Bilkul. Cloud computing ka simple matlab hai internet ke through servers, storage aur computing resources use karna, bina sab kuch khud manage kiye.'
'Simple bolun to AI machine ka smart dimaag jaisa hai, jo examples se seekhta hai.'"""
    )
    ...
    def on_user_input_transcribed(ev):
        ...
        if getattr(ev, "is_final", True):
            ...
            if "sathi" in text_lower and "dost" not in text_lower:
    ...
    # Event listener for when conversation items are added (logging Groq LLM replies)
    ...
    await session.say("Namaste! Main Roxstar AI Dost hoon, batao aaj kya baat karein?", allow_interruptions=True)
...
if __name__ == "__main__":
    ...
    cli.run_app(
        WorkerOptions(
            entrypoint_fnc=entrypoint, agent_name="roxstar-voice-agent"
        )
    )
